...
若只想在本机体验 Web 界面而不想准备配置/证书，可直接运行：

```bash
go run ./cmd/ca-core --demo
```
...
5. 在浏览器中访问 `https://localhost:8443/ui`（默认同样支持 `https://127.0.0.1:8443/ui`）。浏览器可能会提示自签名证书风险，可选择继续访问。使用默认管理员账户（用户名 `admin`）以及日志中输出的初始密码登录，即可通过 Web 界面管理证书的签发、吊销及审计日志。
